function [Results,Qref]=Evaluation_OSP(flg,filename,atyp,tm,ns,lambda,el_sel,ploton)
...
if el_sel==1
   a=(Coord.Z==0);  %logical => index with elements that should be deleted
   %manually keep corner points (points which are part of the beam and
   %column)
...
% special scaling is implemented 
% split the mode shape vector un into its differnet parts (2 translatory DOFs
...
save Sigma.mat sigma lambda Coord
save('./mac calculation/ModalData_FE.mat','atyp','Coord','un','wn') %save for calculation of the MAC metric

Qref=un'*(sigma\un); %Reference FISHER information matrix


%% Perform the OSP-Algorithm
[Results.pos,Results.Q,Results.PHI,Results.H,Results.InfoVec,Results.SensorDof,Results.sigma,Results.accuracy,Results.un,Results.EFI] = OSP_COR_PE(flg,un,wn,ns,sigma);
...
plot_SensorPos(Results,Coord,atyp,ns,length(un(1,:)),jointNo,as,al,flg,lambda)
pause(2)
