function  [sensorDOF,FIM,PHI,H,infoVec,pos,sigma,Accuracy,un,EFI_pareto] = OSP_COR_PE(flg,un,wn,sensorNo,sigma)

% optimalSensorPlacement (Core-function)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Function INPUTs
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
% ETH Zurich
% Copyright 1995-2018 V.K. Ntertimani, C. Leyder
...
%             Orthocheck=psi'*psi; %use only for checking
...
%             minVal = min(DPR,[],2);
        ...
        case 'iei' %via the information entropy
            ...
            for i=1:1:size(sensorDOF,1)
                L=L_new;
                L(i,:)=[];   %remove the sensor at position i and calculate the FIM & IEI of that configuration
    ...
    sigma = [sigma(1:I-1,:);sigma(I+1:end,:)];
    sigma = [sigma(:,1:I-1),sigma(:,I+1:end)];
    informationVector=[informationVector(1:I-1,1);informationVector(I+1:end,1)];
    EFI=[EFI(1:I-1,1);EFI(I+1:end,1)];
    
    EFI_pareto(1:length(EFI),k)=EFI; %#ok<AGROW>
